...
def confirm_account(request):
    ...
    # logger.info(f"Attempting account confirmation with token: {token_value}")

    try:
        token = Token.objects.get(token=token_value, used=False)
        user = token.recipient
        logger.info(f"Token found, user: {user.username}")
        ...
        request.session['user_id'] = user.username # Store user in session
        request.session.modified = True
        login(request, user)  # Log the user in

        return redirect('questionnaire')  # Redirect using named URL
...
#         if not token:
#             return render(request, 'confirm_account.html', {'confirmed': False})
        
#         token_hash = sha256(token.encode()).hexdigest()
#         try:
#             user_profile = get_object_or_404(Participant, confirmation_token=token_hash, token_expiration=timezone.now())
...
#             # Update phase and monitoring start date
#             user_profile.phase = "Physical Activity Monitoring Tomorrow (Wave 1)"
#             user_profile.monitoring_start_date = datetime.date.today() + datetime.timedelta(days=1)
#             user_profile.save()
            
#             return render(request, 'confirm_account.html', {'confirmed': True})
#         except:
#             return render(request, 'confirm_account.html', {'confirmed': False})
...
#     # user = request.user
#     # participant = get_object_or_404(Participant, user=user)
#     # The above code is a Python code snippet with the word "participant" commented out using the "#"
#     # symbol. This means that the word "participant" is not being executed as code and is just a
#     # comment.
#     participant, created = Participant.objects.get_or_create(
#         user=request.user,
#         defaults={
...
#     enrollment_date = participant.monitoring_start_date or timezone.now().date()
#     start_date = enrollment_date + timedelta(days=10)  # Day 11
#     end_date = enrollment_date + timedelta(days=20)    # Day 21
#     current_date = timezone.now().date()
    
#     # day_11 = participant.enrollment_date + timedelta(days=0)
#     # day_21 = participant.enrollment_date + timedelta(days=1)
    
#     within_wave1_period = start_date <= current_date <= end_date
#     user_profile = get_object_or_404(Participant, user=request.user)
...
#         'within_wave1_period': within_wave1_period,
#         'code_error': request.GET.get('code_error', None),
#         'days_until_start': (start_date - current_date).days if current_date < start_date else 0,
#         'days_until_end': (end_date - current_date).days if current_date <= end_date else 0,
...
def questionnaire(request):
    # Get token from either GET or session
    token_value = request.GET.get('token') or request.session.get('token')
    
    # if not token_value:
    #     # Check if user is authenticated first
    #     if request.user.is_authenticated:
    #         # Try to get or create token for authenticated user
    ...
    #     else:
    #         return JsonResponse({'error': 'Token not found and user not authenticated.'}, status=400)
    
    try:
        if request.method == 'GET':
            token = Token.objects.get(token=token_value, used=False)  # Check used=False initially
            user = token.recipient

        elif request.method == 'POST':
            token = Token.objects.get(token=token_value) # No used check here
            user = token.recipient
        ...
        survey = Survey.objects.first()  # Assuming you want to get the first survey
        ...
        user_survey_progress.end_time = datetime.datetime.now()
        user_survey_progress.survey_completed = True
        user_survey_progress.progress_percentage = 100
        user_survey_progress.save()

        token.used = True  # Set used=True *AFTER* successful submission
        token.save()

        if not is_eligible:
            return redirect('exit_screen_not_eligible')

        return redirect(f'/consent-form/?token={token_value}') # Redirect with token
    ...
    #     user_survey_progress, created = UserSurveyProgress.objects.get_or_create(
    #         user=user,
    #         survey=survey,
